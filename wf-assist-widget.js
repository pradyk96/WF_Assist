...
(() => {
  ...
  function plainFallback(question) {
    ...
    if (/(headache|migraine|head pain)/.test(query)) return "For a mild, familiar headache, try water, a small meal if you have not eaten, rest in a dim quiet room, and a break from screens. Only use medicine that is safe for you and follow its label or a clinician’s advice. Seek urgent help for a sudden severe headache, one after an injury, or one with fever, confusion, weakness, vision changes, or a stiff neck.";
    if (/(stress|anxious|anxiety|overwhelm|feeling low|sad)/.test(query)) return "I’m sorry things feel heavy. Try making the next ten minutes smaller: breathe, drink water, name the one thing that matters now, and let the rest wait briefly. If you feel unsafe or might hurt yourself, contact local emergency services or a trusted person immediately. Would it help to talk through what is weighing on you?";
    if (/(idea|brainstorm|creative block|stuck|motivat)/.test(query)) return "Let’s make the problem smaller. Tell me the audience, the feeling you want to create, and the format you are working in; I can turn that into a few practical directions or a simple next-step plan.";
    ...
    return "I can help think through everyday questions, creative ideas, motion design, technology, and the WF portfolio. For a fully open-ended AI conversation, make sure the local server is running with a valid OPENAI_API_KEY. What would you like to talk through?";
  }
